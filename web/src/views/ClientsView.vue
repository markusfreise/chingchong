...
async function archiveClient(client: Client) {
  if (!confirm(`Archive "${client.name}"? This will hide it from active lists.`)) return
  ...
  <div class="page-container">
    ...
    <div v-else class="card">
      <div v-if="clients.length === 0" class="empty-state">
        <p class="empty-state-text">No clients yet. Create one to get started.</p>
      </div>
            ...
            <tr v-for="client in clients" :key="client.id" class="table-row">
              ...
              <td class="table-td">
                <div class="action-btns">
                  <button class="btn-ghost btn-sm" @click="openEdit(client)">Edit</button>
                  <button
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-panel">
        <div class="modal-header">
          <h2 class="heading-2">{{ editingClient ? 'Edit Client' : 'New Client' }}</h2>
